...
basic_irstd_metrics = py_irstd_metrics.ProbabilityDetectionAndFalseAlarmRate(
    num_bins=8,
    distance_threshold=3,
)
opdc_irstd_metrics = py_irstd_metrics.MatchingBasedMetrics(
    num_bins=8,
    matching_method=py_irstd_metrics.OPDCMatching(overlap_threshold=0.5, distance_threshold=3),
)
dist_irstd_metrics = py_irstd_metrics.MatchingBasedMetrics(
    num_bins=8,
    matching_method=py_irstd_metrics.DistanceOnlyMatching(distance_threshold=3),
)
irstd_error_analysis = py_irstd_metrics.HierarchicalIoUBasedErrorAnalysis(
    num_bins=8,
...
class CheckMetricTestCase(unittest.TestCase):
    ...
    def test_pd_fa(self):
        self.assertEqual(curr_results["basic_pd"], self.default_results["dist_pd"])
        self.assertEqual(curr_results["basic_fa"], self.default_results["dist_fa"])

    def test_metrics(self):
        self.assertEqual(curr_results["basic_pd"], self.default_results["basic_pd"])
        self.assertEqual(curr_results["basic_fa"], self.default_results["basic_fa"])

        self.assertEqual(curr_results["dist_fa"], self.default_results["dist_fa"])
        self.assertEqual(curr_results["dist_pd"], self.default_results["dist_pd"])

        self.assertEqual(curr_results["opdc_fa"], self.default_results["opdc_fa"])
        self.assertEqual(curr_results["opdc_pd"], self.default_results["opdc_pd"])
        self.assertEqual(curr_results["opdc_hiou"], self.default_results["opdc_hiou"])

        self.assertEqual(curr_results["seg_iou"], self.default_results["seg_iou"])
        self.assertEqual(curr_results["seg_mrg_err"], self.default_results["seg_mrg_err"])
        self.assertEqual(curr_results["seg_itf_err"], self.default_results["seg_itf_err"])
        self.assertEqual(curr_results["seg_pcp_err"], self.default_results["seg_pcp_err"])
        self.assertEqual(curr_results["loc_iou"], self.default_results["loc_iou"])
        self.assertEqual(curr_results["loc_s2m_err"], self.default_results["loc_s2m_err"])
        self.assertEqual(curr_results["loc_m2s_err"], self.default_results["loc_m2s_err"])
        self.assertEqual(curr_results["loc_itf_err"], self.default_results["loc_itf_err"])
        self.assertEqual(curr_results["loc_pcp_err"], self.default_results["loc_pcp_err"])
